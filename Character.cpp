...
void Character::update(const float & dt, const float & multiplier)
{
	...
	//Items
	this->Object::update(dt, multiplier); //MovementComponent updates
	...
	//Handle colision
	if (this->colisionVec != NULL)
	{
		for (Object* object : *this->colisionVec)
		{
			Wall* wall = dynamic_cast<Wall*>(object);
			if (wall != NULL) {
				wall->updateColisionComponent(&this->sprite, this->hitboxComponent->rect, dt, multiplier);
			}
		}
	}
}
